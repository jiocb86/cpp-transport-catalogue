...
class TransportCatalogue {
    public:
        ...
        void BusesForStop(const std::string_view stop);                                       //добавить автобусы проходящие через остановку
        const std::unordered_set<const Bus*> FindBusesForStop(const Stop* stop) const;        //поиск автобусов проходящих через остановку 
        const std::set<std::string_view> GetStopInfo(const std::string_view stop_name) const; //получить информацию об остановке
        int UniqueStopsCount(const Bus* bus) const;                                           //подсчет уникальных остановок
        double CalculateRouteLength(const Bus* bus) const;                                    //подсчет длины маршрута
    private:
        ...
        std::unordered_map<const Stop*, std::unordered_set<const Bus*>> busname_to_stop_;    //автобусы проходящие через остановку
};
}
